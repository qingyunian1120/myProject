...
public class OutputDialogFragment extends DialogFragment {
    ...
    private void ShowSmal(Boolean b)
    {
        ...
        ViewParent pview = MainApp.mAdViewSmall.getParent();
        if(pview != null)
            ((ViewGroup)pview).removeView(MainApp.mAdViewSmall);

        pview = MainApp.mNativeExpressAdView.getParent();
        if(pview != null)
            ((ViewGroup)pview).removeView(MainApp.mNativeExpressAdView);

       if(b)
       {
           adView1 = MainApp.mAdViewSmall;
           LinearLayoutDialog.addView(adView1,0);
       }
        else
       {
           adView2 = MainApp.mNativeExpressAdView;
